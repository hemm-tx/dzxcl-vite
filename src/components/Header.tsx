    ...
    <>
      <div className="size-full flex justify-center items-center relative">
        <svg className="absolute size-full" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 7680 320">
          ...
          <foreignObject x="0" y="40%" width="100%" height="100%">
          ...
          <foreignObject x="0" y="55%" width="100%" height="30%">
...
const SystemSet = () => {
  const dispatch = useAppDispatch();
  const { mediaMTXStart, isMediaMTXStart, mediaMtxStartLoading, animationDuration } = useAppSelector((state) => state.system);

  const handleAnimationDuration = (e: RadioChangeEvent) => dispatch(setAnimationDuration(e.target.value));
  const handleSwitchMedia = () => dispatch(setMediaMTXStart(!mediaMTXStart));
      ...
      items={[
        {
          ...
          children: <Switch loading={mediaMtxStartLoading} checked={isMediaMTXStart} onChange={handleSwitchMedia} />,
        },
        {
          key: "animationDuration",
          label: <span className="text-white">动画运行速度</span>,
          children: (
            <Radio.Group size="small" value={animationDuration} onChange={handleAnimationDuration} buttonStyle="solid">
